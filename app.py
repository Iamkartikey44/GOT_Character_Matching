...
st.title('Games OF Thrones Personality Matcher')
character = df['character'].values

selected_character = st.selectbox('Select a Character',character)
...
for i in range(len(x)):
    distance.append(np.linalg.norm(x[character_id]-x[i]))

recommended_id = sorted(list(enumerate(distance)),key=lambda x:x[1])[1][0]
